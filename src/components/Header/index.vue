...
export default {
  ...
  methods: {
    ...
    goSearch() {
      // 路由传递参数：（params参数作为路径一部分需要占位，query参数不作为）
      // 第一种：字符串形式
      // this.$router.push('/search/' + this.keyword + '?k=' + this.keyword.toUpperCase())
      // 第二种：模板字符串
      // this.$router.push(`/search/${this.keyword}?k=${this.keyword.toUpperCase()}`)
      // 第三种：对象，需要给路由命名，用路由名称实现跳转
      ...
      // 2:如何指定params参数可传可不传?
      // 如果路由要求传递params参数，但是你就不传通params参数，URL会有问题的
      // 在配置路由的时候，在占位的后面加上个问号【表示params可以传递或者不传递】
      // this.$router.push({name: 'search', query: {k: this.keyword.toUpperCase()}})

      // 面试题3:params参数可以传递也可以不传递，但是如果传递是空串，如何解决?
      // 空串也会导致路径错误，使用undefined解决: params参数可以传递、不传递(空的字符串)
